...
| `generateLeftoverIdeas(recipe)` | `[{name, description, keyTransformation}]` (3 ideas) | 0.8 |
| `generateCookTimeline(recipe)` | `[{stepNumber, label, startMinute, durationMinutes, isParallel}]` | 0.3 |
| `generateSubstitutionMatrix(recipe, dietaryFilter?)` | `[{original, subs: [{name, notes}]}]` (2 subs per ingredient) | 0.5 |
...
| `generateDrinkPairings(recipe)` | `[{type, name, notes}]` (wine/beer/cocktail/non-alcoholic) | 0.7 |
| `generateRecipeDebug(recipe, description)` | `{diagnosis, likelyCause, fix, tip}` | 0.6 |
| `generateCollectionSuggestions(historyNames)` | `[{collectionName, recipeNames[]}]` | 0.7 |
| `generateTagMerges(allTags)` | `[{canonical, duplicates[]}]` | 0.3 |
...
| `daily_challenge` | `lib/challenges.js` — `{ lastCompletedDate: string\|null, streak: number, totalCompleted: number }` |
...
**`MealPlanner.jsx`** — weekly Mon–Sun grid with Breakfast/Lunch/Dinner slots. Uses **tap-to-assign** UX (works on both mobile and desktop): tap a recipe card in the sidebar to select it (orange highlight + checkmark circle), then tap any meal slot to assign it; click anywhere outside to deselect. Slots show an orange dashed border with a `+` icon while a recipe is selected. The status subtitle updates to say which recipe is pending assignment. Shopping list uses `buildSmartShoppingList()` from `lib/shoppingList.js` — deduplicates and groups by aisle; includes `<ShoppingIntegrations>` for HA/Google Tasks. **AI Prep Guide**: "🗓️ Prep Guide" button visible when ≥3 meals assigned; calls `generateMealPrepGuide(meals)` with `{ day, mealType, recipeName, ingredients[], instructions[] }` per meal; displays collapsible sections for each prep day's task checklist, make-ahead items, and a shopping tip; `prepGuide` resets to null whenever `plan` changes (via `useEffect` with `setTimeout(..., 0)`). **Nutrition Overview**: renders `<MealPlanNutrition>` which is purely computed (no API) — shows weekly calorie/macro summary, per-day calorie SVG bar chart, per-day stacked macro bars, and goal comparison when `nutritionGoals` prop is set; displays empty state when < 3 meals assigned. Plan persists in `meal_plan` localStorage key. **AI Fill Plan**: "🤖 AI Fill" button → calls `generateSmartMealPlan(pantryItems, nutritionGoals, prefs)` → shows preview overlay mapping AI suggestions to saved history by name → "Apply" assigns matched entries to slots. **Desktop drag**: recipe cards have `draggable` + `onDragStart` (sets `dataTransfer`); slots have `onDragOver`/`onDrop` with `data-day`/`data-meal` attributes and `data-drop-slot` for targeting; `highlightedSlot` state provides visual drop feedback. **Mobile touch drag**: `handleTouchStart` (150ms press timer triggers ghost), `handleTouchMove` (repositions ghost + uses `elementFromPoint` to find target slot), `handleTouchEnd` (assigns if over `[data-drop-slot]`); ghost element appended to `document.body` with `pointer-events: none`; `dragRef` stores dragged entry, `ghostRef` stores ghost DOM node. Tap-to-assign remains as fallback.
...
**`CookTimeline.jsx`** — collapsible section in ResultView instructions area. Props: `{ recipe }`. Toggle triggers `generateCookTimeline(recipe)` → `[{stepNumber, label, startMinute, durationMinutes, isParallel}]`. Renders SVG Gantt chart: 2 lanes (main=orange / parallel=blue), rounded rect bars per step, time gridlines every 5-10 min, total time label. Mobile: `overflow-x-auto`. Also renders a text step list below the SVG. Cached in component state; `key={recipe.name}` resets on recipe change.

**`SubstitutionMatrix.jsx`** — full-screen modal (`z-[250]`), opened by "🔄 Subs Map" button in ingredient header. Props: `{ recipe, onClose }`. 5 dietary filter pills (All / Vegan / Gluten-Free / Dairy-Free / Nut-Free). Filter switch calls `generateSubstitutionMatrix(recipe, filter)` → `[{original, subs: [{name, notes}]}]`. Results cached in a `Map` ref keyed by filter. Desktop: table layout (Original | Sub 1 | Sub 2); mobile: cards. "Copy All Substitutions" clipboard button.

**`GenerateView.jsx`** — five mode tabs: Ingredients, By Dish Name, Import, Historical, Recreate. Historical tab renders `HistoricalRecipe.jsx`. Recreate tab takes a restaurant name + dish name and calls `onRestaurantGenerate` → `buildRestaurantPrompt`. Mode section also has 🦠 Gut Health and 🌿 Zero-Waste toggles. "Generate Two" button triggers A/B test via `onABGenerate` prop. "🤔 What should I cook?" button calls `generateSmartRecommendation(historyNames, pantryItems, timeOfDay)` and shows the suggested dish as a chip. **"🧺 What Can I Make?"** button (shown when `history.length > 0`) opens `PantryMatcher` modal. **Daily Challenge Card**: `<DailyChallengeCard>` rendered at top of ingredients mode with `onUseIngredient={(ing) => addIngredient(ing)}`.
...
**`PantryDrawer.jsx`** — pantry items are objects `{ name, expiresAt, zone }`. `zone` is `'pantry'` / `'fridge'` / `'freezer'` (defaults to `'pantry'` for legacy items). `normalise()` migrates legacy string items and adds missing `zone`. Zone selector dropdown shown when adding items; zone filter tabs at top filter the list. `ExpiryBadge` renders red/orange/green. **Reorder List** section appears when any items are expired — collapsible, shows expired items with a "Copy list" button. Receipt import adds items to the currently selected zone. **🚨 Expiry Rush** button (shown when ≥1 item expires within 3 days) calls `onAddAll(expiringItems.map(p => p.name))` then `onClose()` to rush all about-to-expire items into the ingredient input. **Round 9 — Ingredient Nutrition Lookup**: ℹ️ button per pantry item → calls `generateIngredientNutrition(name)` → shows a popover with `{calories, protein, carbs, fat, fiber, per}` (per 100g). Popover state: `{ itemName, data, loading }` stored in component state; dismissed by clicking outside.
...
**`PostCookingSummary.jsx`** — modal overlay (`z-[300]`) shown after `onCookDone` fires. Props: `{ recipe, recipeImage, cookingDurationMs, onRate, onLogMeal, onClose }`. Displays recipe image with gradient overlay, elapsed cook time, 5-star rating (hover + click; maps ≥4 → `'up'`, <4 → `'down'` via `onRate`), a "Log to Food Log" button that calls `onLogMeal(mealData)` (extracts name + macros from recipe), leftover storage tips loaded from `generateStorageTips(recipe)`, and **Pantry Depletion** — reads `pantry_items` from localStorage, matches against recipe ingredients (substring match), shows checkboxes for matching items (all checked by default), "Remove Used Items from Pantry" button writes filtered list back. Uses `setTimeout(..., 0)` to defer `setLoadingTips(true)` inside `useEffect` (ESLint `react-hooks/set-state-in-effect` rule).
...
**`RecipeHistory.jsx`** — tabs: All, Saved, Wishlist, Collections, Stats, Journal, Cuisine Passport, Food Log, and **🏆 Trophy**. Accepts `gamification`, `bestStreak`, `nutritionGoals`, `lastRecipe` props from `App.jsx` and forwards the latter two to `DailyFoodLog`, and the former two to `TrophyCase`. **🎲 Random** button in header picks a random history entry and calls `onSelect`. **View toggle** (☰ / ⊞ buttons): `viewMode` state switches between `'cards'` (2-column grid) and `'gallery'` (CSS `columns-2 sm:columns-3` masonry); gallery auto-reverts to cards during remix or multi-select. Gallery shows name strip on mobile always; desktop hover reveals name + difficulty + rating overlay. `MonthlyChallenges` tracks monthly goals. Each card shows version + cook count badges, plus a **Clone** button (calls `onClone(entry.id)`). **Multi-select** toggle (🛒) enables selecting multiple cards; "Merge Shopping Lists" calls `buildSmartShoppingList()`, renders by aisle in the modal, and includes `<ShoppingIntegrations>` for HA/Google Tasks; when exactly 2 selected: "Compare" button opens `<RecipeCompare>` modal. **Remix mode** (🔀 toggle): select up to 2 → "Create Fusion Dish →" calls `onRemix(recipeA, recipeB)`. **Cook-Again Reminder** (`<CookAgainReminder>` before WeeklyChallengeCard): finds saved favourites not cooked in the last 14 days, shows as a nudge widget. **Full-text search** includes `recipe.instructions.join(' ')` in the filter chain (not just name/tags). **Smart Collection Builder**: "✨ AI Suggest" button in Collections tab → calls `generateCollectionSuggestions(historyNames)` → shows suggestion cards with "Create & Assign" button. **Tag Auto-Cleanup**: "🧹 Clean Tags" button → calls `generateTagMerges(allTags)` → shows proposed tag merges with "Apply" button.

**`Navbar.jsx`** — Settings dropdown includes: theme toggle, high contrast, font size (SM/MD/LG), temperature unit (°C/°F), Daily Nutrition Goals (calorie/protein/carbs/fat targets), **Custom Prompt** textarea (appended to every generation), **Integrations** section (collapsible) with Home Assistant URL + token inputs and Google Tasks Client ID input, **Kitchen Tools** section with buttons for Unit Converter, Kitchen Reference, and Backup & Restore. Timer, Planner, Sync, History buttons in the main bar. Props: `history` (for backup), `onImportHistory` (from `importHistory` in `useRecipeHistory`). **Round 9**: `KeyboardShortcutsModal` shortcuts array extended with H (history), P (planner), G (generate), S (save on result view), Escape entries.

**`App.jsx`** keyboard shortcuts — global `keydown` handler (guards `tagName !== 'INPUT' && tagName !== 'TEXTAREA'`): H → `setView('history')`, P → `setView('planner')`, G → `setView('generate')`, S → `handleSave()` (only on result view). **`RecipeMiniPlayer`** rendered after `</main>` when `recipe && view !== 'result' && view !== 'landing' && !miniPlayerDismissed`; `miniPlayerDismissed` resets to `false` on every new recipe (via `useEffect` on `recipe?.name`).
...
**`CookingStats.jsx`** — "Top Ingredients" tab now uses `IngredientFrequency` component. Summary stat cards expanded: Ever Cooked (cookCount > 0), Avg Cal/Serving, Total Cook Time, This Month count. `getTopIngredients` function removed (replaced by IngredientFrequency). **Round 9 additions**: `StreakCalendar` tab (365-day GitHub-style heatmap of cook dates) and `DifficultyHeatMap` tab (monthly calendar colored by difficulty); `<ShoppingStaples history={history} />` rendered below `<IngredientFrequency>`; "🌍 Cuisine Deep-Dive" button opens `<CuisineDeepDive>` modal.

**`GenerateView.jsx`** — additional: `PantryExpiryAlert` shown at top of ingredients mode; quick meal type preset chips (Breakfast/Lunch/Dinner/Snack sets mood); ⚡ `<20 min` speed toggle (sets maxTime to 20); "Clear" button to clear all ingredients when list is non-empty. **Round 9 additions**: "🎰 Roulette" button opens `<IngredientRoulette>` modal (8-slot spin wheel, "Use These!" adds spun ingredients); `<DifficultyRecommender history={history} />` inline after Smart Recommender when `history.length >= 3` (suggests next difficulty level based on cook history).
...
**`StreakCalendar.jsx`** — new (Round 9). Props: `{ history }`. GitHub-style 365-day heatmap: 53-column × 7-row SVG grid; cells colored by cook count on that date (0=slate/1=orange-300/2=orange-400/3+=orange-500); month labels, day labels (M/W/F); tooltip on hover showing date + count; legend strip below.

**`DifficultyHeatMap.jsx`** — new (Round 9). Props: `{ history }`. Monthly calendar colored by difficulty: shows current month with days 1–31 in a 7-column grid; each cell colored by the difficulty of recipes cooked that day (Easy=green / Medium=amber / Hard=red); multiple recipes on same day use highest difficulty; legend strip below.

**`ShoppingStaples.jsx`** — new (Round 9). Props: `{ history }`. Top-10 most-used ingredients as a "pantry staples" list; computed from all history `recipe.ingredients` with same normalization as `IngredientFrequency`; rendered as a ranked chip list with emoji + count badge.

**`DifficultyRecommender.jsx`** — new (Round 9). Props: `{ history }`. Inline suggestion widget shown in GenerateView ingredients mode when `history.length >= 3`. Analyzes recent cook history difficulty distribution; recommends trying Easy if mostly Hard, or trying Hard if mostly Easy. Rendered as a subtle callout chip.

**`AllergyCheck.jsx`** — new (Round 9). Props: `{ ingredients }`. Cross-contamination inline warning rendered in ResultView after description. Reads `pref_allergies` from localStorage (via `useLocalStorage`); performs keyword matching against ingredient strings for common allergens (gluten, dairy, nuts, eggs, soy, shellfish, fish); renders a yellow warning banner listing detected allergens. Returns `null` if no matches or no allergies set.

**`DrinkPairings.jsx`** — new (Round 9). Props: `{ pairings, isLoading, onClose }`. Card component rendered in RecipeActions More panel below the buttons when drink pairings have been loaded. Shows a loading spinner during fetch; renders a grid of pairing cards (wine/beer/cocktail/non-alcoholic) each with `type`, `name`, `notes`; close button dismisses.

**`IngredientRoulette.jsx`** — new (Round 9). Props: `{ onUseIngredients, onClose }`. Full-screen modal with 8-slot spin wheel. "Spin!" button randomly assigns one ingredient from `INGREDIENT_SUGGESTIONS` to each slot with a CSS animation; "Use These!" calls `onUseIngredients(selectedIngredients)` then closes. Each slot has emoji + name.

**`CuisineDeepDive.jsx`** — new (Round 9). Props: `{ onClose }`. Modal cuisine explorer. 8 cuisine chips (Italian/Asian/Mexican/Indian/French/Mediterranean/American/Japanese); selecting one calls `generateCuisineDeepDive(cuisine)` → shows `{description, keyIngredients, techniques, tipForHome, funFact}` in a structured card. Caches results per cuisine in a `Map` ref.

**`MonthlyNutritionReport.jsx`** — new (Round 9). Props: `{ onClose }`. Full-screen modal showing 30-day nutrition summary. Reads `daily_food_log` from localStorage; groups by day; renders: average calories/protein/carbs/fat cards, 30-bar SVG calorie chart (one bar per day, colored vs goal if `pref_nutrition_goals` set), best/worst days, streaks of days with data logged.

**`CookAgainReminder.jsx`** — new (Round 9). Props: `{ history, onSelect }`. Widget showing neglected favourites — saved (`isFavourite`) recipes not cooked (low `cookCount` or no recent cook date) in the last 14 days. Shows up to 3 recipe chips with "Cook again?" button calling `onSelect(entry)`. Returns `null` if no neglected favourites found.

**`RecipeMiniPlayer.jsx`** — new (Round 9). Props: `{ recipe, recipeImage, onGoToRecipe, onDismiss }`. Floating bottom-right card (`fixed bottom-4 right-4 z-[150]`) shown in `App.jsx` when `recipe && view !== 'result' && view !== 'landing' && !miniPlayerDismissed`. Shows recipe thumbnail, name, difficulty. "Go to Recipe →" calls `onGoToRecipe`; ✕ calls `onDismiss` (sets `miniPlayerDismissed` in App.jsx state, reset to `false` on new recipe via `useEffect` on `recipe?.name`).
